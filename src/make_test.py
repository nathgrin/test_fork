...
def main():
    ...
    q.set_options( [ [r"$%.1f\,\mrm{g}$"%(np.random.uniform(0.8,5)) for i in range(5)],
                    ...
                    [r"$%.1f\,\mrm{mL}$"%(np.random.uniform(0.8,5)) for i in range(5)],
                ] )
    ...
    q.set_options( [ [r"$%.1f\,\mrm{%smol L^{\sm1}}$"%(np.random.uniform(0.8,5),np.random.choice(['','k','m',r'\micro'])) for i in range(5)],
                    [r"\ch{Cl}",r"\ch{NO2}",r"\ch{SO4}",r"\ch{CO2}"],
                    [r"$%.1f\,\mrm{%sL}$"%(np.random.uniform(0.8,5),np.random.choice(['','k','m',r'\mu '])) for i in range(5)],
                ] )
    ...
    for i in range(n_tests):
        ...
        out = test.tex_preamble(exc_name,clearpage=toggle,vfill=not toggle) # is not rly a preamble
